...
# Importing the scope-hash module (below) would otherwise make CPython write
# factory/guards/__pycache__/*.pyc next to the source. That is not harmless
...
# and the hash changes with no content change at all -- silently invalidating
# the very review this hook just recorded. Measuring must not alter what is
# being measured, so this process writes no bytecode.
# (The repository's .gitignore covers the same ground for every other tool;
# this line makes the hook correct on its own, including in a project whose
# .gitignore does not.)
